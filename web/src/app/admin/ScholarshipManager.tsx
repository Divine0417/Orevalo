...
import { createScholarship, deleteScholarship, setScholarshipPublished, updateScholarship } from './scholarship-actions'
...
export function ScholarshipRowItem({ scholarship }: { scholarship: ScholarshipRow }) {
  ...
  return <li className={`rounded-2xl border bg-white p-5 ${scholarship.published ? 'border-line' : 'border-dashed border-muted/40 opacity-70'} ${pending ? 'opacity-50' : ''}`}>
    <div className="flex items-start justify-between gap-4 max-sm:flex-col"><div className="min-w-0"><div className="mb-1 flex flex-wrap items-center gap-2"><span className="text-[0.85rem] font-bold text-clay">{scholarship.funder}</span><span className="rounded-full bg-cream-deep px-2.5 py-0.5 text-[0.68rem] font-bold tracking-[0.06em] text-muted uppercase">{scholarship.field}</span>{!scholarship.published && <span className="rounded-full bg-ink/8 px-2.5 py-0.5 text-[0.68rem] font-bold tracking-[0.06em] text-muted uppercase">Hidden</span>}</div><h3 className="font-display text-[1.1rem] font-semibold">{scholarship.name}</h3><p className="mt-1 text-[0.85rem] text-muted">{scholarship.country} · {scholarship.degree_level}{scholarship.deadline ? ` · Deadline ${formatDeadline(scholarship.deadline)}` : ' · Rolling deadline'}</p></div>
      <div className="flex shrink-0 flex-wrap gap-2"><button type="button" disabled={pending} onClick={() => setEditing(true)} className="cursor-pointer rounded-full border-[1.5px] border-line px-4 py-2 text-[0.82rem] font-semibold text-muted hover:border-clay hover:text-clay disabled:cursor-not-allowed">Edit</button><button type="button" disabled={pending} onClick={() => run(() => setScholarshipPublished(scholarship.id, !scholarship.published))} className="cursor-pointer rounded-full border-[1.5px] border-line px-4 py-2 text-[0.82rem] font-semibold text-muted hover:border-clay hover:text-clay disabled:cursor-not-allowed">{scholarship.published ? 'Hide' : 'Publish'}</button>{confirming ? <span className="flex items-center gap-2"><button type="button" disabled={pending} onClick={() => run(() => deleteScholarship(scholarship.id))} className="cursor-pointer rounded-full bg-[#8b3a1a] px-4 py-2 text-[0.82rem] font-bold text-white disabled:cursor-not-allowed">Delete for good</button><button type="button" onClick={() => setConfirming(false)} className="cursor-pointer text-[0.82rem] font-semibold text-muted hover:text-ink">Cancel</button></span> : <button type="button" disabled={pending} onClick={() => setConfirming(true)} className="cursor-pointer rounded-full border-[1.5px] border-[#e07a50]/50 px-4 py-2 text-[0.82rem] font-semibold text-[#8b3a1a] hover:border-[#8b3a1a] disabled:cursor-not-allowed">Delete</button>}</div>
    </div>{error && <p role="alert" className="mt-3 text-[0.85rem] font-semibold text-[#8b3a1a]">{error}</p>}
  </li>
}
