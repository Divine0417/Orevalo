...
The current registry is in `scripts/scraper-sources.mjs`. MyJobMag Nigeria and Scholars4Dev remain enabled for scheduled auditing. MyJobMag detail extraction and the Scholars4Dev detail adapter are implemented and verified in dry-run mode. The remaining documented sources are registered but disabled until their URLs, terms, and extraction fields are audited: NGCareers, Jobberman, Opportunity Desk, Mastercard Foundation, Tony Elumelu Foundation, UNILAG, University of Ibadan, Covenant University, Dangote, MTN Nigeria, Flutterwave, PwC Nigeria, Access Bank, GTBank, and Zenith Bank.
